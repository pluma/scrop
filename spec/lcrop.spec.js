...
describe('scrop.lcrop', function() {
  describe('with an array', function() {
    ...
    it('returns only the first n items', function() {
      expect(scrop([1, 2, 3], 2)).to.eql([1, 2]);
    });
  });
  ...
  if (typeof Buffer !== 'undefined') {
    describe('with a Buffer', function() {
      ...
      it('returns only the first n items', function() {
        expect(scrop(new Buffer([1, 2, 3]), 2)).to.eql(new Buffer([1, 2]));
      });
    });
  }
});